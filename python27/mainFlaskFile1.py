...
users = {'karthikganapathy' : ['karthikganapathy_1127','admin123'],'karthickhariharan' : ['karthickhariharan','admin123'],'ajayraja' : ['ajayraja','test123'],'arunt' : ['arunt','test123'],'gregm' : ['gregm','test123'],'arunapn' : ['arunapn','test123'],'santhoshsa' : ['santhoshsa','test123'],'sivathefire' : ['sivathefire','test123'],'srividyar' : ['srividyar','test123'],'munik' : ['munik','test123'],'prasunv' : ['prasunv','test123'],'subur' : ['subur','test123'],'gautamc' : ['gautamc','test123'],'sujatar' : ['sujatar','test123'],'sanhitas' : ['sanhitas','test123']}
...
def home(var):
  ...
  # Create random Test and train data samples
	...
	try:
		...
		s = """<div style="width:600px; height:700px; position: absolute; top: 20px; left:500px;"><font color="lightcoral"> Error. Something went wrong. Please try again. </font><br><br></div>""" + df[1:15].to_html()
		final_html = template.s1 + "</div>" + s
		return final_html	
  elif (var == 'log_test'):
	try:
...
def chisq1():
  ...
  if request.method == 'POST':
		try:
			...
			temp_df = df[1:15]		
			final_html = template.s1 + " <br><br><u><b> Chi-Square Test Results </b></u><br> Chi-Square statistic: "+ str(fit[0]) + "<br><br> p-value: " + str(fit[1]) + "<br><br> Cross Tabulation <br><br>" + ctab.to_html() + "<br><br></div>" +temp_df.to_html()
			#final_html = template.s1 + "</div> <br><br> Chi-Square Test <br> p-value : "+ str(fit) + "<br><br>" + temp_df.to_html()
			return final_html
		except KeyError:
			final_html = template.s1 + """ <br><br><b> <font color="lightcoral"> Error. Select 2 categorical variables for Chi-square test </font> </b><br><br></div>""" + df[1:25].to_html()
			return final_html
  return 'helloo'
...
def cross1():
  ...
  if request.method == 'POST':
		try:
			...
			temp_df = df[1:15]		
			final_html = template.s1 + " <br><br> Cross Tabulation result <br> : "+ ctab.to_html() + "<br><br></div>" + temp_df.to_html()
			return final_html
...
def linreg2():
  ...
  if request.method == 'POST':
		try:
			...
			print "SUMMARY1",res_lin
			s = (res_lin.summary()).as_html()
			temp_df = df[0:15]	
			t = """</div><div style="width:600px; height:700px; position: absolute; top: 20px; left:500px;"><br> Linear Regression Result <br>"""
...
def bin2():
  ...
  if request.method == 'POST':
		try:
			...
			for key1 in Listkey1:
				...
				if(key1 <> "Create Equal-height Bins" and key1 <> "Create Equal-width Bins" and key1 <> str(bin_val)):
					...
					if(firstkey==1):
						...
						print "Inside column values",df.columns.values
					elif(secondkey==1):	
						print "Inside 222"
			...
			temp_df = df[1:15]
			print "outside column values",df.columns.values
			if(bin_val_fail):
				print "first if"
...
def corr():
  ...
  if request.method == 'POST':
		try:
			...
			corr_res = df1.corr()
			temp_df = df[1:15]		
			final_html = template.s1 + " <br><br> Correlation Matrix <br> "+ corr_res.to_html() + "<br><br></div>" + temp_df.to_html()
			return final_html
		except:
